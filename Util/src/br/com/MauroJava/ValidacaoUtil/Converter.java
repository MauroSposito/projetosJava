...
public class Converter{
	 ...
	 */
	public static byte[] ASCiiParaByteArray(String s, boolean padding){
		int tamanho;
		if (padding){
			tamanho = s.length() % 16;
			if (tamanho == 0) {
	 ...
	 */
	public static String ByteArrayParaHexString(byte[] a){
	 ...
	 */
	public static String HexStringParaString (String s){
		int tamanho = s.length() / 2;
		byte[] arrayResult = new byte[tamanho];
		for (int i = 0; i < tamanho; i++){
			String hex = s.substring(i*2, i*2+2);
